...
export default function Chat() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const response = await fetch(`/api/init-desktop?_t=${Date.now()}`, {
          method: "POST",
          headers: { 
        ...
        if (!response.ok) {
          throw new Error("Failed to initialize desktop");
        }
  ...
  useEffect(() => {
    ...
    const handleKillDesktop = () => {
      const currentSandboxId = session.getSnapshot().sandboxId;
      if (!currentSandboxId) return;
      fetch(`/api/kill-desktop?sandboxId=${currentSandboxId}&_t=${Date.now()}`, {
        method: "POST",
        headers: {
          "Cache-Control": "no-cache",
          "Pragma": "no-cache",
    ...
    if (isIOS || isSafari) {
      window.addEventListener("pagehide", handleKillDesktop);
      return () => {
        window.removeEventListener("pagehide", handleKillDesktop);
        handleKillDesktop();
      };
    } else {
      window.addEventListener("beforeunload", handleKillDesktop);
      return () => {
        window.removeEventListener("beforeunload", handleKillDesktop);
        handleKillDesktop();
      };
    }
